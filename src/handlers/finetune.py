...
class FinetuneHandler:
    ...
    def handle(self, task: Task):
        ...
        pipe.unet.train()
        for i in tqdm(range(0, total_batch_size, consts.BATCH_SIZE),
                    desc="Update",
                    position=2,
                    leave=False, 
                ):
            for each_combination in combinations_list:
                sample_0 = tree.map_structure(lambda value: value[i:i+consts.BATCH_SIZE, each_combination[0]].to(pipe.device), samples)
                sample_1 = tree.map_structure(lambda value: value[i:i+consts.BATCH_SIZE, each_combination[1]].to(pipe.device), samples)
                ...
                for j in tqdm(
                    ...
                    noise_ref_pred_0 = noise_ref_pred_uncond_0 + consts.GUIDANCE_SCALE * (
                    ...
                    noise_ref_pred_1 = noise_ref_pred_uncond_1 + consts.GUIDANCE_SCALE * (
